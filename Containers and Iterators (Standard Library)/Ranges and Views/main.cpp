...
int main() {
    //  create a lambda that accepts two arguments, one of numeric values and the other is a string message. use format to print out with std::cout
    auto showValues {
    ...
    //  filtering and mapping an existing container's elements; data initialization isn't lazy but process pipeline is lazily evaluated, meaning it doesn't process until it's called
